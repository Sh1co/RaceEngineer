...
2. Invoke the "Edit Code" command using one of the following options:
   1. Run the `RaceEngineer: Edit Code 💬` command from the command palette.
   1. Use the "Edit Code" toolbar button in the side panel.
   1. Select the `RaceEngineer > Edit Code 💬` entry in the editor context menu (right-click).
   1. Use the keyboard shortcut: `Ctrl + Cmd + E` (Mac) or `Ctrl + Alt + E` (Windows / Linux).
3. RaceEngineer will generate a diff view.
4. Provide additional instructions to RaceEngineer in the chat thread.
